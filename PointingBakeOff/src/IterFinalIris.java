...
public class IterFinalIris extends PApplet
{
    ...
    ArrayList<Integer> beginsX = new ArrayList<Integer>();
    ArrayList<Integer> beginsY = new ArrayList<Integer>();
    int xpos = 0;
    int ypos = 0;
    ...
    public void mousePressed() // test to see if hit was in target!
    {
        ...
        if ((mouseX > bounds.x && mouseX < bounds.x + bounds.width) && (mouseY > bounds.y && mouseY < bounds.y + bounds.height)) // test to see if hit was within bounds
        {
            ...
            if (trialNum <= 1) {
                ...
                xpos = beginsX.get(beginsX.size() - 2);
                ypos = beginsY.get(beginsY.size() - 2);
            }
            float timeTaken = (millis() - startTime) / 1000f;
            startTime = millis();
//            System.out.println(trialNum + " 1 " + xpos + " " + ypos + " " + bounds.getCenterX()
//                    + " " + bounds.getCenterY() + " " + bounds.width + " " + timeTaken + " 1"); // success
            System.out.println(trialNum + ",2," + xpos + "," + ypos + "," + bounds.getCenterX()
                    + "," + bounds.getCenterY() + "," + bounds.width + "," + timeTaken + ",1"); // success
            hits++;
        } else
        {
            ...
            startTime = millis();
            System.out.println(trialNum + ",2," + xpos + "," + ypos + "," + bounds.getCenterX()
                    + "," + bounds.getCenterY() + "," + bounds.width + "," + timeTaken +",0"); // fail
            misses++;
        }
